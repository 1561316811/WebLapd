...
<body>
	<div class="top">
		<a href="Message.jsp?idUser=<%=idUser%>" class="iconfont">&nbsp;&nbsp;&nbsp;<span>订单</span></a>
	</div>

	<div class="buttom">
		<div style="position:relative;left:15px; top:10px;font-size:20px;margin:0 0 20px 0;">
			<a href="Message.jsp?idUser=<%= idUser %>" 
			style="color:blue;background:#CDC9C9;border-radius: 5px; ">待接收</a> 
			<a href="MessageOrderGet.jsp?idUser=<%= idUser %>">已接收</a>
		...
		<div class="list" style="background:#F2F2F2;border-radius:15px;">
			<div class="item">
				<a
					href="MessageConfirm.jsp?idUser=<%=idUser%>&idOrder=<%=s.getIdOrder()%>"><h4
						class="title"><%=s.getIdRoom()%>房&nbsp;&nbsp;
					</h4>
					<p class="time"><%=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(s.getAddTime())%></p>
					<p class="sum income"><%=s.getClockCatagory()%></p></a>
			</div>
			<hr />
		</div>
		<%
	...
	<footer>
		...
		<a href="Message.jsp?idUser=<%=idUser%>"><i class="iconfont">&#xe75f;<img
				id="msg" src="image/redpoint.png"></img></i>订单</a>
